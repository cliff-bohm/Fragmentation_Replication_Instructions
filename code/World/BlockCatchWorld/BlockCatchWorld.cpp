	...
	if (analyze) {
		...
		*/
		...
		if (save_R_FragMatrix) {
			...
			// new:
			std::vector<std::string> RNames = { "catch/pass", "direction", "size1", "size2", "size3", "size4", "catch & left", "pass & left", "catch & right", "pass & right" };
			std::vector<std::pair<double, double>> flowRanges = { {0,.25}, {.75,1}, {0,1} };//, { 0,.1 }, { .9,1 }};
			FRAG::saveFragMatrixSet(worldStateSet, brainAfterStateSet, lifeTimes, flowRanges, "R_FragmentationMatrix_id_" + std::to_string(thisID) + ".py", "feature", -1);
		}
...
void BlockCatchWorld::evaluate(std::map<std::string, std::shared_ptr<Group>>& groups, int analyse, int visualize, int debug) {
	...
	//			std::ios::app);
			...
			for (auto ms : mutantScores){
				FileManager::writeToFile("mutantScoreFile.txt",std::to_string(ms));
			}
