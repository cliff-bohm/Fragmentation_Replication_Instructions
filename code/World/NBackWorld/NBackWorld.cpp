...
void NBackWorld::evaluateSolo(std::shared_ptr<Organism> org, int analyze, int visualize, int debug) {
	...
	if (analyze) {
		...
		if (saveFlowMatrix) {
			std::cout << "  life times:         " << lifeTimes.size() <<   "  life time o duration : " << lifeTimes[0] << std::endl;
                        std::cout << "  time series length: " << brainStates.size() << "  time series entry length: " << brainStates[0].size() << std::endl;
